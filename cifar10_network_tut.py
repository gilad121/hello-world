...
def model():
    ...
    fc = tf.nn.relu(tf.layers.dense(inputs=flat, units=100)) #, activation=tf.nn.relu)
    drop4 = tf.nn.dropout(fc, keep_prob2)
    fc2 = tf.nn.relu(tf.layers.dense(inputs=drop4, units=50))
